...
export default function Search() {
    ...
    return (
        <section className="w-full min-h-screen flex flex-col gap-3 p-8 mt-8">
            <Input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search for a post..."/>
            {loading&&"Loading..."}
            ...
            : results.map((post)=> {
                return <li key={post.title} className="bg-gray-500/10 p-3 rounded hover:scale-[1.01] transition-all duration-200">
                   ...
                   <p className="text-gray-300 text-sm">{post.excerpts.substring(0, 30)}</p>
                   <div className="flex items-center gap-2 text-xs mt-3">
                        Written by:
                        <Image alt={"img"} src={post.author.image} width={10} height={10} />
